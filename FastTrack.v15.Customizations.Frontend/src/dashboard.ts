...
export class FastTrackDashboard extends UmbElementMixin(LitElement) implements UmbDashboardElement {
	...
	async #requestExpiredMembers() {
		const { data } = await tryExecuteAndNotify(this, SubscriptionsApiService.getUmbracoSubscriptionsApiV1GetExpired());
	...
	render() {
		return html` ${this.#renderActiveMembers()} ${this.#renderExpiredMembers()} `;
	}
			...
			${repeat(
				...
				(member) =>
					html`<uui-ref-node-member
						...
						@open=${() => this.#onOpenMember(member)}></uui-ref-node-member>`
			)}</uui-box
		>`;
	}
